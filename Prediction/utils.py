...
# Load TFLite models
def load_maize_model(model_path="Prediction/models/maizemodel.tflite"):
    maize_interpreter = tf.lite.Interpreter(model_path=model_path)
    maize_interpreter.allocate_tensors()
    return maize_interpreter

# def load_crop_model(model_path="Prediction/models/crop_recommendation_model.tflite"):
#     crop_interpreter = tf.lite.Interpreter(model_path=model_path)
#     crop_interpreter.allocate_tensors()
#     return crop_interpreter

def preprocess_image(image_url, target_size=(256, 256)):
...
# def predict_crop(crop_interpreter, sensor_data):
#     input_details = crop_interpreter.get_input_details()
#     output_details = crop_interpreter.get_output_details()
#     input_shape = input_details[0]['shape']
#     input_data = np.array([sensor_data['nitrogen'], sensor_data['phosphorus'], sensor_data['potassium'], sensor_data['pH'], sensor_data['electroconductivity'], sensor_data['moisture']], dtype=np.float32)
#     if input_data.shape!= input_shape:
#         raise ValueError("Input tensor shape doesn't match the expected shape.")
#     input_data = input_data.reshape(input_shape)
#     crop_interpreter.set_tensor(input_details[0]['index'], input_data)
...
def predict_disease(image_url, maize_interpreter):
    input_data = preprocess_image(image_url)
    
    input_details = maize_interpreter.get_input_details()
    output_details = maize_interpreter.get_output_details()

    maize_interpreter.set_tensor(input_details[0]['index'], input_data)
    maize_interpreter.invoke()
    output_data = maize_interpreter.get_tensor(output_details[0]['index'])
